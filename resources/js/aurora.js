...
class Aurora {
    constructor(element, options = {}) {
        this.ctn = typeof element === 'string' ? document.querySelector(element) : element;
        if (!this.ctn) return;

        this.options = {
            colorStops: options.colorStops || ['#091632', '#2F6CF9', '#275ACF'],
            amplitude: options.amplitude !== undefined ? options.amplitude : 1.5,
            blend: options.blend !== undefined ? options.blend : 0.6,
    ...
    resize() {
        if (!this.ctn || !this.renderer) return;
    ...
    destroy() {
        ...
        if (this.ctn && this.renderer && this.renderer.gl.canvas.parentNode === this.ctn) {
...
// Automatically instantiate on DOM load if we are on the landing page/hero is present
document.addEventListener('DOMContentLoaded', () => {
    const heroBg = document.querySelector('.aurora-hero-bg');
    if (heroBg) {
        new Aurora(heroBg, {
            colorStops: ['#091632', '#2F6CF9', '#275ACF'],
            blend: 0.6,
            amplitude: 1.6,
            speed: 0.45
        });
